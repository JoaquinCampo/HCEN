...
class ClinicAccessPolicyMDBTest {
    ...
    private ClinicAccessPolicyMDB mdb;

    @BeforeEach
    void setUp() throws Exception {
        mdb = new ClinicAccessPolicyMDB();

        // Inject the mocked service using reflection
        Field serviceField = ClinicAccessPolicyMDB.class.getDeclaredField("accessPolicyService");
    ...
    void onMessage_ShouldProcessValidMessage() throws Exception {
        // Given
        String payload = "12345678|Hospital Central|REQ-001";
        when(textMessage.getText()).thenReturn(payload);

        AddClinicAccessPolicyDTO expectedDto = new AddClinicAccessPolicyDTO();
        expectedDto.setHealthUserCi("12345678");
        expectedDto.setClinicName("Hospital Central");
        expectedDto.setAccessRequestId("REQ-001");

        // When
        mdb.onMessage(textMessage);

        // Then
        verify(accessPolicyService).createClinicAccessPolicy(argThat(dto -> {
            return "12345678".equals(dto.getHealthUserCi()) &&
                    "Hospital Central".equals(dto.getClinicName()) &&
    ...
    void onMessage_ShouldProcessValidMessageWithNullAccessRequestId() throws Exception {
        // Given
        String payload = "12345678|Hospital Central|";
        when(textMessage.getText()).thenReturn(payload);
    ...
    void onMessage_ShouldHandleServiceException() throws Exception {
        ...
        String payload = "12345678|Hospital Central|REQ-001";
        when(textMessage.getText()).thenReturn(payload);
        when(accessPolicyService.createClinicAccessPolicy(any())).thenThrow(new RuntimeException("Service error"));

        // When
        mdb.onMessage(textMessage);

        // Then
        verify(accessPolicyService).createClinicAccessPolicy(any());
    }
